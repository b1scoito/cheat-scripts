...
-- making chockedcommands global for paint event
...
client.set_event_callback(
    ...
    function()
        ...
        local y = renderer.indicator(220, 220, 220, 255, string.format("lag %d%%", percentage))
        local w, h = renderer.measure_text("b", string.format("lag %d%%", percentage))

        -- what the fuck even are these numbers
        renderer.circle_outline(w + 70, y + 19, 0, 0, 0, 150, 10, 0, 1, 5)
        renderer.circle_outline(w + 70, y + 19, 220, 220, 220, 255, 9, 0, percentage / 100, 4)
    end
)
...
client.set_event_callback(
    "shutdown",
    function()
        -- setting both hidden elements invisible on shutdown

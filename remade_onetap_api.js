...
var api = {
    globals: {
        ...
        framestage: function () {
            return Globals.FrameStage()
        } // Returns an integer value of the current frame stage. https://github.com/ValveSoftware/source-sdk-2013/blob/master/sp/src/public/cdll_int.h#L135
    },
    ui: {
        add_label: function (name) {
            return UI.AddLabel(name);
        }, // Adds a text label.
        toggle_hotkey: function () {
            return UI.ToggleHotkey(arguments);
        }, // Used to toggle hotkey. 1 = Active, 0 = Inactive
        add_textbox: function (name) {
            return UI.AddTextbox(name);
        }, // Adds a textbox.
        set_color: function () {
            return UI.SetColor(arguments);
        }, // Set color of color picker.
        add_color_picker: function (name) {
            return UI.AddColorPicker(name);
        }, // Adds a color picker.
        add_multi_dropdown: function (name, items) {
            return UI.AddMultiDropdown(name, items);
        ...
        add_dropdown: function (name, items) {
            return UI.AddDropdown(name, items);
        }, // Adds a dropdown.
        add_hotkey: function (name) {
            return UI.AddHotkey(name);
        }, // Adds a label with a key picker control.
        add_slider_float: function (name, min_val, max_val) {
            return UI.AddSliderFloat(name, min_val, max_val);
        }, // Adds a floating-point slider.
        add_slider_int: function (name, min_val, max_val) {
            return UI.AddSliderInt(name, min_val, max_val);
        }, // Adds an integer slider.
        add_checkbox: function (name) {
            return UI.AddCheckbox(name);
        }, // Adds a checkbox control.
        is_hotkey_active: function () {
            return UI.IsHotkeyActive(arguments);
        }, // Returns whether a key is currently active or not.
        get_color: function () {
            return UI.GetColor(arguments);
        }, // Returns an array of color values from a control.
        get_string: function () {
            return UI.GetString(arguments);
        }, // Returns a string representation of an item's current value.
        set_enabled: function () {
            return UI.SetEnabled(arguments);
        }, // Changes menu item(s) visibility.
        set_value: function (value) {
            return UI.SetValue(arguments, value);
        }, // Sets the value of an UI item's setting.
        get_value: function () {
            return UI.GetValue(arguments)
    ...
    render: {
        ...
        polygon: function (x, y, x1, y1, x2, y2, r, g, b, a) {
            return Render.Polygon(
                x, y,
                x1, y1,
                x2, y2, r, g, b, a);
        }, // Draws a polygon with shape based on arguments passed.
        text_size: function (text, size, optional) {
            return Render.TextSize(text, size, optional);
        ...
        gradient_rect: function (x, y, w, h, dir, color1, color2) {
            return Render.GradientRect(x, y, w, h, dir, color1, color2);
    ...
    cheat: {
        ...
        callback: function (callback, fun) {
            return Cheat.RegisterCallback(callback, fun);
        ...
        List of events:
         ...
         Draw: Where you call render functions.
         Unload: Called when script gets unloaded.
...
api.cheat.log("wow, much cool");

function on_create_move() {
    api.cheat.log("Real time: " + api.globals.realtime());
}

api.cheat.callback("CreateMove", "on_create_move");
